...
namespace MidiPlayerTK
{
    ...
    public class SoundFontSetupWindow : EditorWindow
    {
...
#if MPTK_PRO
        public static string version = "1.8 Pro";
#else
        public static string version = "1.8 Free";
#endif
        public static string releaseDate = "July 19 2018";
        ...
        [MenuItem("Tools/MPTK - SoundFont Setup")]
        ...
        void OnGUI()
        {
            try
            {
                ...
                GUIContent content = new GUIContent() { text = "Setup SoundFont in your application - Version " + version, tooltip = "" };
                ...
                ShowListBanks(startx + widthLeft + espace, starty, widthRight, heightTop);

                ShowOptim(startx, starty + espace + heightTop, widthLeft, heightMiddle + heightBottom + espace);
                ShowLogOptim(startx + widthLeft + espace, starty + heightTop + espace, widthRight, heightMiddle + heightBottom + espace);
                ...
                if (MidiPlayerGlobal.CurrentMidiSet != null && MidiPlayerGlobal.CurrentMidiSet.SoundFonts != null)
                {
                    ...
                    for (int i = 0; i < MidiPlayerGlobal.CurrentMidiSet.SoundFonts.Count; i++)
                    {
                        ...
                        if (GUI.Button(new Rect(boxX, boxY + 9, 80, buttonHeight), "Select"))
                        {
                            ...
                            ToolsEditor.LoadImSF(sf.ImSFPath, sf.Name);
        ...
        private void ShowOptim(float localstartX, float localstartY, float width, float height)
        {
            try
            {
                ...
                if (MidiPlayerGlobal.ImSFCurrent != null)
                {
                    ...
                    {
                        EditorGUI.LabelField(new Rect(xpos, ypos, 380 + 85, itemHeight), "No patchs and waves has been extracted yet from the Soundfont.", styleRed);
                        ypos += itemHeight / 2f;// + espace;
                        EditorGUI.LabelField(new Rect(xpos, ypos, 380 + 85, itemHeight), "Now, you have to choose between Optimize or Extract all.", styleRed);
                        ypos += itemHeight;// + espace;
                    }
                    else
                    {
                    ...
                    */
...
#if MPTK_PRO
                    if (GUI.Button(rect, new GUIContent("Optimize from Midi file list", "You need to setup some midi files before to launch ths optimization")))
                    {
                        KeepAllPatchs = false;
                        KeepAllZones = false;
                    ...
                    if (GUI.Button(rect, new GUIContent("Extract all Patchs & Waves", "")))
                    {
                        KeepAllPatchs = true;
                        KeepAllZones = true;
                        OptimInfo = new BuilderInfo();
